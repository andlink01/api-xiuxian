...
class TelegramClient:
        ...
        while True:
            try:
                ...
                if not self.app.is_connected:
                     ...
                     await asyncio.sleep(self.COMMAND_DELAY_SECONDS * 2)
                     self.command_queue.task_done(); continue
                ...
                try:
                    if not self.command_queue.empty(): self.command_queue.task_done()
                except ValueError: logger.warning("【消息队列】严重错误处理中调用 task_done 失败。")
                except Exception as td_err: logger.error(f"【消息队列】严重错误处理中调用 task_done 时发生未知错误: {td_err}")

    def _is_from_game_bot(self, message: Message) -> bool:
        """检查消息是否来自配置的游戏机器人ID之一"""
        if not self.game_bot_ids: return False
        return (message.sender_chat and message.sender_chat.id in self.game_bot_ids) or \
               (message.from_user and message.from_user.id in self.game_bot_ids)
    ...
    def register_handlers(self):
        ...
        if self.game_bot_ids and self.target_chat_id:
             ...
             logger.info(f"游戏消息监听器: 监听 {len(self.game_bot_ids)} 频道/用户在群组 {self.target_chat_id}。")
    ...
    async def send_game_command(self, command: str) -> bool:
        ...
        try:
            ...
            logger.info(f"【消息队列】指令 '{safe_log_preview_q}...' 已加入队列 (当前队列大小: {self.command_queue.qsize()})。")
            return True
        except Exception as e:
