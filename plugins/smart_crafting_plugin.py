...
class Plugin(BasePlugin):
    ...
    # --- (修改结束) ---
        ...
        # --- (修改: 传递 quantity) ---
        task = asyncio.create_task(self._execute_smart_crafting(message, item_name, quantity, edit_target_id, task_id))
        # --- (修改结束) ---
        self._active_crafting_tasks[task_id] = task
        task.add_done_callback(lambda t: self._active_crafting_tasks.pop(task_id, None))

    # --- (修改: 增加 quantity 参数) ---
    async def _execute_smart_crafting(self, message: Message, item_name: str, quantity: int, edit_target_id: int | None, task_id: str):
    # --- (修改结束) ---
        """执行智能炼制的后台任务"""
        crafter_id = await get_my_id(self, message, edit_target_id)
        ...
        if not crafter_id or not crafter_username:
            await edit_or_reply(self, message.chat.id, edit_target_id, "❌ 无法获取当前助手信息，无法执行。", original_message=message)
            return
        ...
        if not self.data_manager or not redis_client or not self.context.telegram_client or not self.context.event_bus:
            await edit_or_reply(self, message.chat.id, edit_target_id, "❌ 内部错误：缺少核心服务。", original_message=message)
            return

        # 0. 获取物品主数据
        await edit_or_reply(self, message.chat.id, edit_target_id, f"⏳ 正在加载基础数据...", original_message=message)
        item_master = await self.data_manager.get_item_master_data(use_cache=True)
        if not item_master:
            await edit_or_reply(self, message.chat.id, edit_target_id, "❌ 无法获取物品主数据，无法继续。", original_message=message)
            return
        target_item_id = await get_item_id_by_name_local(item_name, item_master)
        target_recipe_item_id = f"recipe_{target_item_id}" if target_item_id and not target_item_id.startswith("recipe_") else target_item_id
        target_item_details = item_master.get(target_item_id) if target_item_id else None
        if target_item_details and target_item_details.get("type") == "recipe":
             target_recipe_item_id = target_item_id

        if not target_item_id:
             await edit_or_reply(self, message.chat.id, edit_target_id, f"❌ 在物品主数据中找不到物品 '{item_name}'。", original_message=message)
             return
        if not target_recipe_item_id:
             self.warning(f"无法确定物品 '{item_name}' (ID: {target_item_id}) 对应的配方物品 ID，将跳过学习检查。")

        # 1. 获取配方
        await edit_or_reply(self, message.chat.id, edit_target_id, f"⏳ 正在查找 '{item_name}' 的配方...", original_message=message)
        recipe_materials = await get_recipe(redis_client, item_name)
        if not recipe_materials:
            await edit_or_reply(self, message.chat.id, edit_target_id, f"❌ 未找到物品 '{item_name}' 的配方。请先使用 `,更新配方` 添加。", original_message=message)
            return

        # 2. 配方学习检查 (逻辑不变)
        if target_recipe_item_id:
            await edit_or_reply(self, message.chat.id, edit_target_id, f"⏳ 正在检查 {crafter_username} 是否已学习配方 '{item_name}'...", original_message=message)
            ...
            if learned_recipes_list is None:
                await edit_or_reply(self, message.chat.id, edit_target_id, f"❌ 无法获取 {crafter_username} 已学习的配方列表。", original_message=message); return
            if target_recipe_item_id not in learned_recipes_list:
                self.warning(f"助手 {crafter_username} 未学习配方 '{item_name}' (ID: {target_recipe_item_id})。检查背包...")
                await edit_or_reply(self, message.chat.id, edit_target_id, f"⚠️ {crafter_username} 未学习配方，正在检查背包...", original_message=message)
                inventory_data_learn = await self.data_manager.get_inventory(crafter_id, use_cache=False)
                recipe_item_found = await find_recipe_item_in_inventory(inventory_data_learn, target_recipe_item_id)
                if recipe_item_found:
                    recipe_item_name = item_master.get(target_recipe_item_id, {}).get("name", item_name)
                    learn_command = LEARN_RECIPE_COMMAND_FORMAT.format(recipe_item_name)
                    self.info(f"找到配方物品 '{recipe_item_name}'，准备发送学习指令 '{learn_command}'...")
                    await edit_or_reply(self, message.chat.id, edit_target_id, f"⏳ 找到配方物品，正在发送学习指令 `.学习 {recipe_item_name}`...", original_message=message)
                    learn_success = await self.context.telegram_client.send_game_command(learn_command)
                    if learn_success: await edit_or_reply(self, message.chat.id, edit_target_id, f"✅ 配方未学习，已找到配方物品并发送学习指令。\n请在学习成功后重新尝试炼制。", original_message=message)
                    else: await edit_or_reply(self, message.chat.id, edit_target_id, f"❌ 配方未学习，找到配方物品但发送学习指令失败！", original_message=message)
                    return
                else:
                    self.error(f"助手 {crafter_username} 未学习配方 '{item_name}' 且背包中无此配方。")
                    await edit_or_reply(self, message.chat.id, edit_target_id, f"❌ 炼制失败：{crafter_username} 未学习配方 '{item_name}'，且背包中也未找到该配方。", original_message=message)
                    return
            else: self.info(f"助手 {crafter_username} 已学习配方 '{item_name}'。")
        else: self.warning(f"无法确定配方物品 ID for '{item_name}'，跳过学习检查。")
        # --- 配方学习检查结束 ---
        ...
        for check_attempt in range(3): # 最多尝试3轮收集
            ...
            await edit_or_reply(self, message.chat.id, edit_target_id, f"⏳ 第 {check_attempt + 1} 次检查 {crafter_username} 的材料 (炼制 {quantity} 个)...", original_message=message, throttle_duration=5)

            inventory_data = await self.data_manager.get_inventory(crafter_id, use_cache=False) # 强制刷新背包
            if not inventory_data:
                await edit_or_reply(self, message.chat.id, edit_target_id, f"❌ 无法获取 {crafter_username} 的背包信息。", original_message=message)
                return
            ...
            if has_enough:
                ...
                await edit_or_reply(self, message.chat.id, edit_target_id, f"✅ {crafter_username} 材料足够 (炼制 {quantity} 个)，准备发送炼制指令...", original_message=message, throttle_duration=5)
            ...
            await edit_or_reply(self, message.chat.id, edit_target_id, f"⚠️ {crafter_username} 材料不足 (炼制 {quantity} 个)，缺少：\n{missing_text}\n⏳ 正在检查其他助手库存...", original_message=message, throttle_duration=5)
            ...
            if final_shortfall: # 如果最终缺口字典不为空 (所有助手加起来都不够)
                ...
                await edit_or_reply(self, message.chat.id, edit_target_id, f"❌ 所有助手库存总和不足！(炼制 {quantity} 个)\n最终缺少: {final_missing_report}\n炼制任务 '{item_name}' 已停止。", original_message=message)
                return # 任务失败，退出

            # --- 总库存足够，触发收货流程 ---
            materials_to_request = missing_materials # 需要请求的就是当前缺少的
            self.info(f"库存足够，准备为缺少的材料发布 {len(materials_to_request)} 个转账请求...")
            await edit_or_reply(self, message.chat.id, edit_target_id, f"⏳ 其他助手库存足够，正在为缺少的 {len(materials_to_request)} 种材料发布转账请求...", original_message=message, throttle_duration=5)

            request_channel = self.config.get("marketplace_transfer.request_channel")
            if not self.context.redis or not request_channel:
                 await edit_or_reply(self, message.chat.id, edit_target_id, "❌ 无法发布转账请求：Redis 或请求频道未配置。", original_message=message); return
            ...
            if not pay_item_id_default:
                 await edit_or_reply(self, message.chat.id, edit_target_id, f"❌ 无法获取默认支付物品 '{pay_item_name_default}' 的 ID。", original_message=message); return
            ...
            await edit_or_reply(self, message.chat.id, edit_target_id, f"⏳ 已发布 {publish_success_count} 个材料请求，失败 {publish_fail_count} 个。\n⏳ 等待材料到账 (最多 {MATERIAL_WAIT_TIMEOUT} 秒)...", original_message=message, throttle_duration=5)
            ...
            while asyncio.get_event_loop().time() - wait_start_time < MATERIAL_WAIT_TIMEOUT:
                ...
                await edit_or_reply(self, message.chat.id, edit_target_id, f"⏳ 等待材料到账 ({remaining_time} 秒)...", original_message=message, throttle_duration=10)
            ...
            if not materials_arrived: # 超时
                ...
                await edit_or_reply(self, message.chat.id, edit_target_id, f"❌ 等待材料超时！(炼制 {quantity} 个)\n最终仍缺少:\n{final_missing_text}\n炼制任务 '{item_name}' 已停止。", original_message=message)
                return # 任务失败，退出

            # 材料已到账，继续下一次外层循环检查（将在下次循环开始时成功并 break）
            self.info("材料收集完成，进入下一轮检查确认。")
            await edit_or_reply(self, message.chat.id, edit_target_id, f"⏳ 材料收集完成，正在进行最终确认...", original_message=message, throttle_duration=5)
        # --- 材料检查与收集循环结束 ---
        ...
        if materials_gathered:
            ...
            if queue_success:
                await edit_or_reply(self, message.chat.id, edit_target_id, f"✅ 材料已集齐！炼制指令 `{craft_command}` 已成功加入队列。", original_message=message)
            else:
                await edit_or_reply(self, message.chat.id, edit_target_id, f"❌ 材料已集齐，但将炼制指令 `{craft_command}` 加入队列失败！", original_message=message)
        else:
             # 理论上应该在收集循环中因为 final_shortfall 或超时而返回了
             await edit_or_reply(self, message.chat.id, edit_target_id, f"❌ 未知错误：材料收集流程结束但未确认集齐 (炼制 {quantity} 个)。最终缺少: {final_missing_report or '未知'}", original_message=message)
