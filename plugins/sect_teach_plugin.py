...
REDIS_TEACH_LOCK_KEY = "sect_teach:action_lock"
REDIS_PENDING_PLACEHOLDER_KEY_PREFIX = "sect_teach:pending_placeholder"
# --- 新增：记录当天是否已发送完成通知的 Key ---
REDIS_TEACH_COMPLETED_NOTIFY_KEY = "daily_notify_sent:sect_teach"
# --- 新增结束 ---
...
async def _check_sect_teach():
    ...
    redis_client = context.redis.get_client(); lock_acquired = False
    if redis_client:
        try:
            lock_acquired = await redis_client.set(REDIS_TEACH_LOCK_KEY, "1", ex=60, nx=True)
            if not lock_acquired: logger.info("【自动传功】获取检查锁失败，上次检查或操作可能仍在进行中，跳过本次。"); return
        except Exception as e: logger.error(f"【自动传功】检查或设置 Redis 锁失败: {e}，跳过本次。"); return
    else: logger.error("【自动传功】Redis 未连接，无法检查锁，任务终止。"); return
    try:
        my_id = context.telegram_client._my_id
        my_username = context.telegram_client._my_username # 获取用户名
        if not my_id: logger.warning("【自动传功】无法获取助手 User ID，跳过本次检查。"); return
        ...
        if needs_teach:
            ...
            if redis_client:
                ...
                try:
                    # 使用 setnx 检查今天是否已发送过通知，并设置25小时过期
                    notify_sent = await redis_client.set(notify_key, "1", ex=25*3600, nx=True)
                    if notify_sent: # 如果 setnx 返回 True，说明是今天第一次设置成功
        ...
        if lock_acquired and redis_client:
            try: await redis_client.delete(REDIS_TEACH_LOCK_KEY)
            except Exception as e_lock: logger.error(f"【自动传功】释放检查锁时出错: {e_lock}")
...
class Plugin(BasePlugin):
    ...
    def register(self):
        ...
        try:
            ...
            # 移除 trigger_character_sync_now 监听器，因为同步由 handle_game_response 触发
            # self.event_bus.on("trigger_character_sync_now", self.handle_manual_sync_trigger)
    ...
    # 移除 handle_manual_sync_trigger 方法
    # async def handle_manual_sync_trigger(self):
    #     self.info("【自动传功】收到事件: trigger_character_sync_now")
    ...
    async def handle_game_response(self, message: Message, is_reply_to_me: bool, is_mentioning_me: bool):
        ...
        if not self.auto_enabled or not self.context.data_manager or not self.context.event_bus or not self.context.redis: return # 确保所需服务可用
        ...
        # 必须是以上两种回复之一才继续处理
        if not is_success_reply and not is_fail_or_limit_reply:
            return # 忽略不相关的回复

        # 确认回复的是传功指令 (通过检查回复的消息 ID 是否在最近发送的传功指令 ID 记录中 - 此处简化，假设最近的回复就是对的)
        teach_cmd_msg_id = message.reply_to_message_id
        # (可以增加更严格的验证，比如从 Redis 读取最近发送的传功指令 ID)
        ...
        if is_success_reply and current_done_count > 0:
            if current_done_count < 3:
                ...
                send_completion_notify = True # 标记发送完成通知
            ...
            if "次数已用完" in text or "已达上限" in text or "过于频繁" in text:
                 ...
                 send_completion_notify = True # 次数用尽也算完成
            else:
                 sync_reason = "传功失败" # 失败不发送完成通知，等待下次检查重试
