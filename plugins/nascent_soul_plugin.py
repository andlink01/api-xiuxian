...
# Redis Keys
ACTION_LOCK_KEY = "nascent_soul:action_lock" # 操作锁 (防止并发发送指令)
ACTION_LOCK_TTL = 120 # 2分钟
WAITING_STATUS_KEY_PREFIX = "nascent_soul:waiting_status:" # 等待状态回复
...
async def _send_status_check_command(is_startup_check: bool = False):
    ...
    if not context or not context.telegram_client or not context.redis or not context.scheduler or not context.data_manager: # 增加 data_manager
    ...
    if not char_status:
        ...
        # 即使无法获取缓存，也继续尝试发送状态检查，让 handle_response 处理（可能需要手动干预）
    else:
        ...
        if not current_realm:
            ...
            # 不需要释放锁，因为还没获取
            # 可以在这里移除定时任务，但更简单的是每次检查都判断
            # 如果希望只在第一次发现不满足时提示一次，需要增加 Redis 标记
            return # 直接返回，不执行后续逻辑
        else:
    ...
    try:
        lock_acquired = await redis_client.set(ACTION_LOCK_KEY, "1", ex=ACTION_LOCK_TTL, nx=True)
        if not lock_acquired:
            logger.info(f"{log_prefix} 获取操作锁失败，上次操作可能仍在进行中，跳过。")
            return

        logger.info(f"{log_prefix} 成功获取操作锁，正在发送 '{CMD_STATUS}' 指令...")
        await redis_client.set(wait_key, "1", ex=WAITING_STATUS_TTL)
        ...
        if lock_acquired and redis_client:
             logger.debug(f"{log_prefix} 任务异常或发送失败，释放锁。")
             await redis_client.delete(ACTION_LOCK_KEY)
...
async def _handle_status_timeout():
    ...
    await redis_client.delete(ACTION_LOCK_KEY)
    logger.info("【自动元婴】[超时] 已清理等待标记和操作锁。")
            ...
            if is_waiting_status:
                ...
                if REGEX_STATUS_NOURISHING.search(text):
                    ...
                    if countdown_sec > 0:
                        ...
                        if not success: await redis_client.delete(lock_key)
                ...
                lock_acquired_settle = await redis_client.set(ACTION_LOCK_KEY, "1", ex=ACTION_LOCK_TTL, nx=True)
                if lock_acquired_settle:
                    self.info("【自动元婴】(归窍后) 成功获取锁，发送 '.元婴出窍' 指令。")
                    # --- (新增: 发送出窍前再次检查境界) ---
                    can_egress_settle = False
                    char_status_settle = await self.context.data_manager.get_character_status(self._my_id, use_cache=True) # 使用缓存即可
                    if char_status_settle:
                        ...
                        can_egress_settle = True # 无法确认时，允许尝试
